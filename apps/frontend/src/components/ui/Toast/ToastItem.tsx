...
export function ToastItem({ toast }: ToastItemProps) {
  ...
  return (
    <div
      className={`
        ${bgColor} ${textColor} border-l-4 p-4 rounded shadow-md
          ...
          {toast.action && (
            <button
              ...
              className={`mt-2 px-2 py-1 text-sm font-medium rounded-md
                ${toast.type === 'success' ? 'bg-green-200 text-green-800 hover:bg-green-300' : ''}
                ${toast.type === 'error' ? 'bg-red-200 text-red-800 hover:bg-red-300' : ''}
                ${toast.type === 'warning' ? 'bg-yellow-200 text-yellow-800 hover:bg-yellow-300' : ''}
                ${toast.type === 'info' ? 'bg-blue-200 text-blue-800 hover:bg-blue-300' : ''}
              `}
            >
              {toast.action.label}
            </button>
