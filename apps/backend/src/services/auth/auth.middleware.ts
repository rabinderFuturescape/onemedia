...
export class AuthMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const userData = await this._authClientService.validateToken(auth);
      ...
      const user = await this._userService.getUserById(userData.id);
      ...
      if (user?.isSuperAdmin && impersonate) {
        ...
        if (loadImpersonate) {
          const impersonatedUser = loadImpersonate.user;
          impersonatedUser.isSuperAdmin = true;
          delete impersonatedUser.password;
          ...
          loadImpersonate.organization.users =
            loadImpersonate.organization.users.filter(
              (f) => f.userId === impersonatedUser.id
            );
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-expect-error
