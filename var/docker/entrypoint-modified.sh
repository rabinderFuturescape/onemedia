...
# Skip Prisma DB push if SKIP_PRISMA is set to true
if [[ "$SKIP_PRISMA" != "true" ]]; then
        echo "Entrypoint: Running database migrations"
        npm run prisma-db-push
else
        echo "Entrypoint: Skipping database migrations (SKIP_PRISMA=true)"
fi
...
# If POSTIZ_APPS is empty, run the static server
if [[ "$POSTIZ_APPS" == "" ]]; then
        echo "Entrypoint: Running static server"
        exec /app/run-static-server.sh
else
